...
namespace Pms.Adjustments.ServiceLayer.Files
{
    public class BillingRecordImporter
    {
        ...
        public static ICollection<BillingRecord> Import(string filePath)
        {
            var records = new List<BillingRecord>();
            using var nNewPayreg = new FileStream(filePath, FileMode.Open, FileAccess.Read);
            var nWorkBook = new HSSFWorkbook(nNewPayreg);
            var nSheet = nWorkBook.GetSheetAt(0);
            var nRow = nSheet.GetRow(0);
            int rIdx = 1;

            while (nRow != null || nRow?.GetCell(3) != null)
            {
                try
                {
                    nRow = nSheet.GetRow(rIdx);
                    if (nRow == null || nRow.GetCell(3) == null) break;

                    var newRecord = new BillingRecord
                    {
                        AdjustmentOption = AdjustmentOptions.ADJUST2
                    };

                    if (!Enum.TryParse(nRow.GetCell(0).StringCellValue, out DeductionOptions deductionOption))
                    {
                        rIdx += 1;
                        continue;
                    }
                    ...
                    if (!Enum.TryParse(nRow.GetCell(1).StringCellValue, out AdjustmentTypes adjustmentType))
                    {
                        rIdx += 1;
                        continue;
                    }
