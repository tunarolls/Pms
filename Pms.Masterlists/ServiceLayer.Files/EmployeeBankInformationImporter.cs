...
namespace Pms.Masterlists.ServiceLayer.Files
{
    ...
    {
        public static IEnumerable<IBankInformation> StartImport(string fileName)
        {
            ...
            while (i <= nSheet.LastRowNum)
            {
                ...
                var bankInfo = new Employee
                {
                    EEId = row.GetCell(1).GetValue(formulator)
                };
